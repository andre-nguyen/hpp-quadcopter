...
#ifndef POLYNOMIAL_OPTIMIZATION_IMPL_H_
#define POLYNOMIAL_OPTIMIZATION_IMPL_H_
...
void PolynomialOptimization<_N>::setupMappingMatrix(
  ...
  for (int i = 0; i < N / 2; ++i) {
    Polynomial<N>::baseCoeffsWithTime(_A.template row(i), i, 0);
    Polynomial<N>::baseCoeffsWithTime(_A.template row(i + N / 2), i,
                                      segment_time);
  }
}
...
double PolynomialOptimization<_N>::computeCost() const {
  ...
  for (size_t segment_idx = 0; segment_idx < n_segments_; ++segment_idx) {
    ...
    const Segment<N>& segment = segments_[segment_idx];
    for (size_t dimension_idx = 0; dimension_idx < dimension_;
         ++dimension_idx) {
      const typename Polynomial<N>::VectorR c =
          segment[dimension_idx]
              .template getCoefficients<derivative_order::POSITION>();
...
void PolynomialOptimization<_N>::updateSegmentTimes(
  ...
  for (size_t i = 0; i < n_segments_; ++i) {
    ...
    Polynomial<N>::quadraticCostJacobian(cost_matrices_[i], segment_time,
...
void PolynomialOptimization<_N>::computeSegmentMaximumMagnitudeCandidates(
  ...
  if (segment.getDimension() > 1) {
    ...
    for (const Polynomial<N>& p : segment.getPolynomialsRef()) {
      Eigen::Matrix<double, n_d, 1> d =
          p.template getCoefficients<Derivative>().transpose();
      Eigen::Matrix<double, n_dd, 1> dd =
          p.template getCoefficients<Derivative + 1>().transpose();
      convolved_coefficients += convolve(d, dd);
    }

    Polynomial<convolved_coefficients_length> polynomial_convolved;
  ...
  // compute the roots of the derivative.
    ...
    const Polynomial<n_d> d(
        segment[0].template getCoefficients<Derivative>().transpose());
...
void PolynomialOptimization<_N>::
    computeSegmentMaximumMagnitudeCandidatesBySampling(
        const Segment<N>& segment, double t_start, double t_stop, double dt,
        std::vector<double>* candidates) {
...
Extremum PolynomialOptimization<_N>::computeMaximumOfMagnitude(
  ...
  int segment_idx = 0;
  Extremum extremum;
  for (const Segment<N>& s : segments_) {
